...
interface CalendarFourDayViewProps {
  currentDate: Date;
...
const CalendarFourDayView: React.FC<CalendarFourDayViewProps> = ({ currentDate, onEventClick, allEvents, isLoadingEvents }) => {
  console.log('CalendarFourDayView - currentDate:', currentDate);
  console.log('CalendarFourDayView - allEvents:', allEvents);
  console.log('CalendarFourDayView - isLoadingEvents (from context):', isLoadingEvents);
  ...
  const totalEventsInView = days.reduce((count, day) => count + getEventsForDay(day).length, 0);
    ...
    <div className="bg-surfaceContainer rounded-lg shadow-md p-6 h-full flex flex-col">
      {isLoadingEvents ? (
        ...
        <div className="flex flex-1 items-center justify-center text-onSurfaceVariant">
          No events for these days.
        </div>
      ) : (
        <div className="flex-1 overflow-x-auto">
          <div className="grid grid-cols-[50px_repeat(4,_minmax(120px,_1fr))] gap-1 h-full w-max md:w-full">
...
export default CalendarFourDayView;